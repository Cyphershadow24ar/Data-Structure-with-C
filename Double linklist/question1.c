//WAP a code in c using data strucutre - Double linklist
...
{
    int n;   
    printf("ENTER THE NO. OF NODES U WANT TO INSERT: ");
    scanf("%d", &n);
    printf("\n");
    for (int i = 1; i <= n; i++)
    {
...
void insany(struct node *x)
{
    ...
    struct node *q;
    q = malloc(sizeof(struct node));
    printf("ENTER AFTER WHICH DATA U WANT TO INSERT: ");
    scanf("%d", &n);
    printf("ENTER THE DATA TO INSERTED: ");
    scanf("%d", &q->data);
    while (x != NULL)
...
void delete()
{
    ...
    printf("\n");
    printf("WHICH TYPE OF DELETION U WANT TO USE: ");
    scanf("%d", &n);
    switch (n)
    {
    case 1:
